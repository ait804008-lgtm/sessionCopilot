...
public final class SessionStoreImpl: SessionStore {
    ...
    public func deleteSession(_ id: UUID) async throws {
        // Capture the audioFilePath before removing the session so we
        // can clean up the WAV file after the JSON is saved.
        let audioPath = sessions.first(where: { $0.id == id })?.audioFilePath
        sessions.removeAll { $0.id == id }
        save()
        AudioStorage().delete(audioFilePath: audioPath)
    }

    public func deleteSessions(_ ids: Set<UUID>) async throws {
        guard !ids.isEmpty else { return }
        // Capture audio paths for cleanup.
        let audioPaths = sessions
        ...
        for path in audioPaths {
            storage.delete(audioFilePath: path)
        }
    }

    public func deleteAllSessions() async throws {
        let audioPaths = sessions.map { $0.audioFilePath }
        sessions.removeAll()
        save()
        let storage = AudioStorage()
        for path in audioPaths {
    ...
    /// Also deletes the associated audio files.
    ...
    public func deleteSessionsOlderThan(days: Int) async throws -> Int {
        ...
        let audioPaths = toDelete.map { $0.audioFilePath }
        ...
        if count > 0 {
            ...
            // Also clean up orphaned audio files (no session in the store).
            // This handles files left behind by crashes or manual edits.
