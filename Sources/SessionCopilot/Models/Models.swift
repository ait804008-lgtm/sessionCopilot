...
public struct Session: Codable, Sendable, Identifiable {
    ...
    /// Optional + Codable-tolerant so old sessions without audio decode cleanly.
...
public struct AppSettings: Codable, Sendable {
    ...
    // MARK: - Codable (backward-compatible decoding for older persisted settings)

    /// Custom decoder that tolerates missing keys for fields added after
    /// the initial release. Older persisted `AppSettings` JSON lacking
    /// `semanticDetectionEnabled`, `silenceThreshold`, or
    /// `audioRecordingEnabled` will decode with defaults instead of
    /// throwing and falling back to `AppSettings()` (which would discard
    /// the user's existing hotkeys, API keys, opacity, etc.).
