...
# Copy prompt templates (behavioral, coding, classification) into the
# bundle so `PromptLoader.loadAndRender` can find them via Bundle.main
# when running from /Applications. Without this, the app falls back to
# `FileManager.default.currentDirectoryPath/prompts/...` which is only
# valid during `swift test`.
if [[ -d "$REPO_ROOT/prompts" ]]; then
    cp -R "$REPO_ROOT/prompts" "$APP_ROOT/Contents/Resources/prompts"
    echo ">> Copied prompt templates into bundle"
else
    echo "WARNING: prompts/ directory not found — PromptLoader will use inline fallbacks" >&2
fi
